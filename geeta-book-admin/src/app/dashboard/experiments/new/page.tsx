...
import { createVerseActivity, getChapters, getVerses } from '@/lib/api';
...
export default function NewVerseActivityPage() {
  ...
  useEffect(() => {
    if (formData.chapterId) {
      ...
      setFormData((prev) => ({ ...prev, verseNumber: '' }));
    } else {
      setVerses([]);
    }
  }, [formData.chapterId]);
  ...
  const loadVerses = async (chapterId: string) => {
    try {
      ...
      console.log('Loaded verses:', data);
      setVerses(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error creating verse activity:', error);
      alert('Failed to create verse activity');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Create Verse Activity</h1>
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            Add a new verse-specific activity, experiment, or practice
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          >
            {loading ? 'Creating...' : 'Create Verse Activity'}
          </button>
          <Link
            href="/dashboard/experiments"
